...
export function CompactModuleCard({ id, label, expanded, onExpandedChange, children }: CompactModuleCardProps) {
    const contentId = `compact-module-${id}`
    return (
        <section data-testid={`compact-card-${id}`} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
