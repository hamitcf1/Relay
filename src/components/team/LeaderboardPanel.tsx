...
export function LeaderboardPanel() {
    ...
    const getRankIcon = (rank: number) => {
        switch (rank) {
            ...
            case 2: return <Medal className="w-6 h-6 text-zinc-400 fill-zinc-400/20" />
            case 3: return <Medal className="w-6 h-6 text-amber-600 fill-amber-600/20" />
            default: return <span className="text-zinc-500 font-bold w-6 text-center tabular-nums">#{rank}</span>
        }
    }

    const getRowStyle = (rank: number) => {
        if (rank === 1) return "bg-gradient-to-r from-yellow-500/10 to-transparent border-l-4 border-l-yellow-500"
        if (rank === 2) return "bg-gradient-to-r from-zinc-500/10 to-transparent border-l-4 border-l-zinc-500"
        if (rank === 3) return "bg-gradient-to-r from-amber-600/10 to-transparent border-l-4 border-l-amber-600"
        return "bg-zinc-900/40 border border-zinc-800 hover:bg-zinc-800/50"
    }
                    ...
                    <div className="flex bg-zinc-900 p-1 rounded-lg border border-zinc-800">
                        <button
                            ...
                            className={cn(
                                ...
                                timeRange === 'day' ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "text-zinc-400 hover:text-white"
                            )}
                        >
                            {t('leaderboard.today')}
                        </button>
                        <button
                            ...
                            className={cn(
                                "px-4 py-1.5 text-sm font-medium rounded-md transition-all",
                                timeRange === 'week' ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "text-zinc-400 hover:text-white"
                            )}
                        >
            ...
            <CardContent className="p-6 space-y-4 overflow-y-auto max-h-[calc(100vh-250px)] custom-scrollbar">
                {loading ? (
                    <div className="space-y-4">
                        {[1, 2, 3, 4, 5].map(i => (
                            <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-zinc-900/50 animate-pulse">
                        ...
                        >
                            <div className="flex items-center gap-5">
                                ...
                                <div className="flex items-center gap-4">
                                    <Avatar className="h-12 w-12 border-2 border-white/5 ring-2 ring-transparent group-hover:ring-indigo-500/30 transition-all">
                                    ...
                                    <div>
                                        <p className="font-bold text-white text-lg group-hover:text-indigo-400 transition-colors">
                                            {entry.userName}
                                        </p>
                            ...
                            <div className="flex flex-col items-end gap-1">
                                <div className="flex items-baseline gap-1">
                                    ...
                                    <span className="text-2xl font-bold font-mono text-white tabular-nums ml-2">
                                        {entry.totalMinutes % 60}
                                    </span>
                                    <span className="text-sm font-medium text-zinc-500">m</span>
                                </div>
                                <div className="flex items-center gap-1.5 text-[10px] uppercase font-bold text-zinc-600 tracking-wider">
