...
export type PriorityQueueItem = { id: string; title: string; meta: string; time: Date; rank: number; label: 'critical' | 'high' | 'pinned' | 'payment' }
...
export function buildPriorityQueue(notes: ShiftNote[], sales: Sale[]): PriorityQueueItem[] {
    ...
    const saleItems = sales.filter((sale) => sale.status !== 'cancelled' && sale.payment_status !== 'paid' && sale.total_price > sale.collected_amount).map((sale): PriorityQueueItem => ({
        id: `sale-${sale.id}`, title: sale.name, meta: sale.room_number ? `room:${sale.room_number}` : sale.customer_name, time: sale.updated_at || sale.created_at, rank: 3, label: 'payment',
    }))
