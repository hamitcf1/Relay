...
export function PriorityQueue({ items, copy, onOpen }: { items: PriorityQueueItem[]; copy: Record<string, string>; onOpen: () => void }) {
    return <section aria-labelledby="priority-work-title" className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
        <header className="flex items-center justify-between gap-4 border-b border-border/60 px-4 py-4 sm:px-5"><div><h2 id="priority-work-title" className="text-base font-semibold tracking-tight">{copy.priority}</h2><p className="mt-0.5 text-xs text-muted-foreground">{copy.priorityDescription}</p></div><button onClick={onOpen} className="shrink-0 text-xs font-semibold text-primary hover:underline">{copy.allRecords}</button></header>
